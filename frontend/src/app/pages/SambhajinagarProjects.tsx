...
export default function SambhajinagarProjects() {
  const [selectedProject, setSelectedProject] = useState<Project | null>(null);
  const { projects } = useProjects();
  const cityProjects = projects.filter((p) => p.city === 'sambhajinagar');
      ...
      <section className="pt-32 pb-16 px-6 lg:px-12 bg-[#F8F8F8]">
        <div className="max-w-[1200px] mx-auto text-center">
          <span className="text-[#f3e218] tracking-[3px] mb-4 block" style={{ fontSize: '11px', fontWeight: 400 }}>
            PORTFOLIO — CHHATRAPATI SAMBHAJI NAGAR
          </span>
          <h1 className="text-[#2B2B2B]" style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 700, letterSpacing: '0.02em', lineHeight: 1.2 }}>
            Projects in Chhatrapati Sambhaji Nagar
          </h1>
          <p className="text-[#2B2B2B]/70 mt-4 max-w-xl mx-auto" style={{ fontSize: '1rem', fontWeight: 400, lineHeight: 1.8 }}>
            Kapadnekar Design Consultancy has executed residential and commercial projects in Chhatrapati Sambhaji Nagar, bringing considered design and quality construction to this growing region.
          </p>
        </div>
      </section>
      ...
      {selectedProject && (
        <section className="py-32 px-6 lg:px-12">
          <div className="max-w-[1200px] mx-auto">
            ...
            >
              ← Back to Chhatrapati Sambhaji Nagar Projects
            </button>
